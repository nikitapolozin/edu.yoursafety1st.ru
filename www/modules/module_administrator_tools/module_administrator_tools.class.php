...
class module_administrator_tools extends EfrontModule {
	...
	private function doFixSharedFolders() {
		...
		$sharedFoldersForm -> addElement('static', '', 'Возникли проблемы с доступом к файлам из уроков, которые имеют общие папки с другими уроками? Нажмите эту кнопку, чтобы повторно синхронизировать общие папки. Возможно, вам придется запускать несколько раз (пока не будет 0 обновлений)');
		$sharedFoldersForm -> addElement('checkbox', 'content', 'Обновить контент');
	...
	private function doChangeFileEncoding() {
		...
		$fileEncodingsForm -> addElement('select', 'encoding', _MODULE_ADMINISTRATOR_TOOLS_SELECTENCODINGCONVERSION, array('из UTF-8 в UTF7-IMAP', ' из UTF7-IMAP в UTF-8'));
