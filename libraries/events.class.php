...
//2222222222222222222222222
   ...
   /**
    ...
    public function appendNewNotification($event_types, $replace_notification = false, $create_negative = true, $delete_negative = false) {
    	...
    	} else if ($this->event['send_interval']) {
    		// ticket #6484 editing an "after" notification causes to search all notifications of this type.. this is to limit the clause to only after/before notifications
    		$event_notifications = eF_getTableData("event_notifications", "*", "active = 1 AND after_time != 0 AND (event_type = '".$this -> event['type'] ."')");
		} else {
